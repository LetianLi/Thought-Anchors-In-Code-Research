...
from thought_anchors_code.config import (
    DATA_DIR,
    canonical_dataset_name as normalize_dataset_name,
    local_model_dir,
    resolve_dataset_dir,
)
...
DEFAULT_TOKENIZER_PATH = local_model_dir()
...
def render_distribution_section(
    title: str, metric_rows: list[tuple[str, dict[str, Any]]]
) -> list[str]:
    if not metric_rows:
        return []
...
def analyze_humaneval(
    ...
    solution_tokens = batched_token_counts(
        tokenizer, (row["canonical_solution"] for row in rows)
    )
...
def analyze_mbpp(
    ...
    setup_code_tokens = batched_token_counts(
        tokenizer, (row["test_setup_code"] for row in rows)
    )
    ...
    lines = render_kv_section(
        ...
        [
            ...
            (
                "task id range",
                f"{min(row['task_id'] for row in rows)}..{max(row['task_id'] for row in rows)}",
            ),
...
def top_prefixes(task_ids: Iterable[str], prefix_split: str = "/") -> str:
    prefixes = [
        task_id.split(prefix_split, 1)[0]
        for task_id in task_ids
        if prefix_split in task_id
    ]
    if not prefixes:
        return "n/a"
...
def load_rows(dataset_dir: Path) -> list[dict[str, Any]]:
    dataset_name = dataset_dir.name if dataset_dir.name != "human_eval" else "openai_humaneval"
...
def analyze_dataset(
    ...
    dataset_dir = resolve_dataset_dir(dataset_name, data_dir)
    ...
    if not rows:
        return "\n".join(
            render_dataset_header(dataset_name, "n/a") + ["Overview", "  rows  0"]
        )

    lines = render_dataset_header(
        dataset_name, top_prefixes(str(row["task_id"]) for row in rows)
    )
...
def main() -> None:
    ...
    reports = [
        analyze_dataset(dataset_name, args.data_dir, tokenizer)
