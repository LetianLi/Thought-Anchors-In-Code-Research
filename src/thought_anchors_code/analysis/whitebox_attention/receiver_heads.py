...
def get_trace_vertical_scores(
    ...
    scores = np.zeros((layers, heads, len(sentences)), dtype=np.float32)
    for layer in range(layers):
        for head in range(heads):
            scores[layer, head, :] = get_vertical_scores(
                matrices[layer, head],
...
def rank_receiver_heads(
    ...
    layers, heads = mean_kurtosis.shape
    ranked = []
    for flat_index in top_flat:
        ...
        ranked.append(
            ReceiverHead(
                layer=layer, head=head, kurtosis=float(mean_kurtosis[layer, head])
            )
        )
    return ranked
